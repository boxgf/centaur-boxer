...
#define scoreOfBlock -float(1000*costOfForward)
...
#define maxdepth 4
#define discount 0.8
#define bonus 1000*costOfForward
#define blockDistanceFactor 100
...
class Position
{
      ...
      char operator-(Position second)
      {

      second=Position(second.P[0]-P[0]+1,second.P[1]-P[1]+1);
...
class State;
Queue<char> makePath(State*, Position);
...
public:
   ...
   void placeBlock(Queue<Position> &blocks)
   {
      Position popped;
   
      while(blocks.isThere())
      {
    
      popped=blocks.pop();
      if(Bot_State[popped.P[1]][popped.P[0]] !='X'&&Bot_State[popped.P[1]][popped.P[0]] !='-')
      Bot_State[popped.P[1]][popped.P[0]] ='B';
      }
   }
   int blockDistance()
   {
   ...
   {
   Queue<char> temp;
   temp=makePath(this,Position(xSize+1,ySize+1));
   int n=temp.count();
   temp.freeIt();
   return(n);
   }
...
Position retPos=botPos;
 ...
 switch (botDirection)
  ...
  return retPos;
    ...
    for(int i=0;i<ySize;i++)
    for(int j=0;j<xSize;j++)
    if (Bot_State[i][j]=='.'||Bot_State[i][j]=='2'||Bot_State[i][j]=='3'||Bot_State[i][j]=='B')
    return 1;
    return 0;
    }
   ...
   score+=scoreOf3;
   else if(Bot_State[i][j]=='B')
   score+=scoreOfBlock;  
   
    ////cout<<score-cost;
    if(isBlock())
    return (score-float(cost)/60.0-blockDistanceFactor*blockDistance());
    return (score-float(cost)/60.0-nearestPelleteDistance());
    }
    State()
    {
    botDirection='0';
...
if((newState.Bot_State[i.P[1]][i.P[0]]<='4'&& newState.Bot_State[i.P[1]][i.P[0]]>j)||newState.Bot_State[i.P[1]][i.P[0]]=='.')
...
Queue<char> makePath(State *inputState, Position destination=Position(xSize,ySize))
{
State clean=*inputState;
bool nearFlag=0;;
if(destination==Position(xSize+1,ySize+1))
nearFlag=1;
inputState->clean();
Position currentPos,posToPush,poppedChild,*temp;
Queue<Position> orderQ,child;
Queue<Position*> clearMemory;
...
while(orderQ.isThere()&&((!(currentPos==destination)&&!nearFlag)||(nearFlag&&(clean.Bot_State[currentPos.P[1]][currentPos.P[0]]!='.')&&(clean.Bot_State[currentPos.P[1]][currentPos.P[0]]!='2')&&(clean.Bot_State[currentPos.P[1]][currentPos.P[0]]!='3') )))
{
currentPos=orderQ.pop();
if(!(destination==Position(xSize,ySize)))
{
temp=(class Position*)malloc(sizeof(Position));
*temp=currentPos;
clearMemory.push(temp);
}
...
{
if(temp->parent!='\0')
pathQ.append(*(temp->parent)-*temp);
temp=temp->parent;              
}
while(clearMemory.isThere())
free(clearMemory.pop());
return(pathQ);
}
...
for(int i=0;i<ySize;i++)                                                       //change this check if node is there
for(int j=0;j<xSize;j++)
Maze.Bot_State[i][j]='-';
Maze.botPos=Position(0,0);
//NODES HERE

makePath(&Maze);//fillpelletes
